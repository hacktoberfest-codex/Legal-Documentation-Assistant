...
async function will(personData, req, res) {
    ...
    //     temp[0] = temp[0].replace(`[beneficiary0.name]`, beneficiary[0].name);
    //     temp[0] = temp[0].replace(`[beneficiary0.address]`, beneficiary[0].address);
    // }
    // else {
    for (let i = 0; i < temp.length; i++) {
        temp[i] = temp[i].replace(`[beneficiary${i}.name]`, beneficiary[i].name);
        temp[i] = temp[i].replace(`[beneficiary${i}.address]`, beneficiary[i].address);
        if (i != temp.length - 1) {
            temp[i] += '.';
        }
        benefitStatement += `${i + 1}. ${temp[i]} \n`
    }
    // }
